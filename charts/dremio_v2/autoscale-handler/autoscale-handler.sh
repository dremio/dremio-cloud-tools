...
function validateInput {
    ...
    if [[ ! "${action}" =~ $(echo ${validActions[*]} | tr ' ' '|') ]]; then
        echo "$action is not a valid option. Please select one of the following: ${validActions[@]}"
        exit 1
    fi
...
function sendRequest {
    ...
    if [[ -z "${request}" ]]; then
        echo "Failed to send request to the Dremio API."
        exit 3
    fi
...
function getActiveFragments {
    ...
    if [[ "${request}" != *"value"* ]]; then
        echo "No results recevied from Prometheus. Prometheus Query: ${fragmentActiveQuery}"
        exit 5
    fi
...
function pollPrometheusActiveFragments {
    ...
    do
        ...
        then
            echo "Max retries exceeded while waiting for queries to complete."
            exit 6
        else
